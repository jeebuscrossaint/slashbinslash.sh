...
try {
  ...
  console.log(
    "dotenv not available, using defaults or system environment variables",
  );
}

const PORT = process.env.PORT || 3000;
const NC_PORT = process.env.NC_PORT || 9999;
const HOST = process.env.HOST || "localhost";
const UPLOAD_DIR = process.env.UPLOAD_DIR || path.join(__dirname, "uploads");
const MAX_FILE_SIZE = parseInt(process.env.MAX_FILE_SIZE || 100 * 1024 * 1024); // 100MB
const DEFAULT_EXPIRY_DAYS = parseInt(process.env.DEFAULT_EXPIRY_DAYS || 7);
const MAX_EXPIRY_DAYS = parseInt(process.env.MAX_EXPIRY_DAYS || 14);
...
app.post("/upload", uploadLimiter, (req, res) => {
  ...
  upload.single("file")(req, res, (err) => {
    ...
    // Create response data with simpler URL format
    const responseData = {
    ...
    if (isCommandLine) {
      ...
      const fullUrl = `${req.protocol}://${HOST}:${PORT}/${responseData.url}`;
...
app.get("/up.sh", (req, res) => {
  ...
  if [ -t 0 ]; then
      ...
      result=$(curl -s -F "file=@$1" $PROTOCOL://$SERVER/upload)
  else
      # If input is from a pipe
      echo "Uploading from pipe..."
      result=$(curl -s -F "file=@-" $PROTOCOL://$SERVER/upload)
  fi
